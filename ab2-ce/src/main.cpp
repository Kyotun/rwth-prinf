...
void vAufgabe_5(){
	...
	weg_ptr1->vAnnahme(std::move(fahrzeug4),3);
...
void vAufgabe_6(){
	...
	unique_ptr<Weg> hinweg = make_unique<Weg>("Hinweg", 500.0);
	unique_ptr<Weg> rueckweg = make_unique<Weg>("Rueckweg", 500.0, Innerorts);
	...
	// Zeichnen der Straße
	bZeichneStrasse(hinweg->getName(), rueckweg->getName(), hinweg->getLaenge(), 2, koordinaten);
	...
	for(dGlobaleZeit = dEpsilon; dGlobaleZeit < 4; dGlobaleZeit += dEpsilon){
		...
		// Kontrolliere die Fahrzeuge fuer Tanken, die aufm Hinweg fahren.
		for(const auto& fahrzeug : *hinweg->getFahrzeugList()){
			//Kontrolliere ob die TankZeit gekommen ist.
			if(fmod(dGlobaleZeit,dTankZeit) < dEpsilon){
				fahrzeug->dTanken(fahrzeug->getTankvolumen());
			}
		...
		// Kontrolliere die Fahrzeuge fuer Tanken, die aufm Rueckweg fahren.
		for(const auto& fahrzeug : *rueckweg->getFahrzeugList()){
			//Kontrolliere ob die TankZeit gekommen ist.
			if(fmod(dGlobaleZeit,dTankZeit) < dEpsilon){
				fahrzeug->dTanken(fahrzeug->getTankvolumen());
			}
...
void vAufgabe_7(){
	...
	Kreuzung::vVerbinde("W12", "W21", 40, Kr1, Kr2, Innerorts);
	Kreuzung::vVerbinde("W23a", "W32a", 115, Kr2, Kr3, Autobahn);
	Kreuzung::vVerbinde("W23b", "W32b", 40, Kr2, Kr3, Innerorts);
	Kreuzung::vVerbinde("W24", "W42", 55, Kr2, Kr4, Innerorts);
	Kreuzung::vVerbinde("W34", "W43", 85, Kr3, Kr4, Autobahn);
	Kreuzung::vVerbinde("W44a", "W44a", 130, Kr4, Kr4, Landstrasse);
